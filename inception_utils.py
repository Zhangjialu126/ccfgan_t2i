...
def prepare_inception_metrics(dataset, parallel, no_fid=False, label=None):
    ...
    if type(label) == int:
        data_mu = np.load('intra_inceptions/'+dataset + '_{:03d}_inception_moments.npz'.format(label))['mu']
        data_sigma = np.load('intra_inceptions/'+dataset + '_{:03d}_inception_moments.npz'.format(label))['sigma']
    else:
