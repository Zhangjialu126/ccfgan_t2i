...
def run(config):
    ...
    train_loaders, test_loaders = utils.get_data_loaders(**config)
    # Load inception net
    net = inception_utils.load_inception_net(parallel=config['parallel'])
    pool, logits, captions, CLIP_tokens, keys = [], [], [], [], []
    device = 'cuda'
    for i, (x, caption, CLIP_token, key) in enumerate(tqdm(test_loaders[0])):
        x = x.to(device)
        with torch.no_grad():
            pool_val, logits_val = net(x)
            pool += [np.asarray(pool_val.cpu())]
            logits += [np.asarray(F.softmax(logits_val, 1).cpu())]
            captions += [np.asarray(caption)]
            CLIP_tokens += [np.asarray(CLIP_token.cpu())]
            keys += [np.asarray(key)]

    pool, logits, captions, CLIP_tokens, keys = [np.concatenate(item, 0) for item in [pool, logits, captions, CLIP_tokens, keys]]
...
def run_intra(config):
  ...
  for cls in range(nclass_dict[config['dataset']]):
    ...
    mu, sigma = np.mean(pool, axis=0), np.cov(pool, rowvar=False)
    print('Saving calculated means and covariances to disk...')
    np.savez('intra_inceptions/'+config['dataset'].strip('_hdf5')+'_{:03d}_inception_moments.npz'.format(cls), **{'mu' : mu, 'sigma' : sigma})
