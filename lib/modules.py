...
from lib.cf_loss_function import CFLossFunc


############   GAN   ############
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, iteration, writer, args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            ...
            real_output = netC(real_feats, sent_emb)
            # pred_real, errD_real, real_output = predict_loss(netC, real_feats, sent_emb, negtive=False)
            # real_output = netC(real_feats, sent_emb)
            # predict mismatch
            mis_sent_emb = torch.cat((sent_emb[1:], sent_emb[0:1]), dim=0).detach()
            real_mismatch_output = netC(real_feats, mis_sent_emb)
            # _, errD_mis, real_mismatch_output = predict_loss(netC, real_feats, mis_sent_emb, negtive=True)
            # real_mismatch_output = netC(real_feats, mis_sent_emb)
            ...
            fake_output = netC(fake_feats, sent_emb)
            # _, errD_fake, fake_output = predict_loss(netC, fake_feats, sent_emb, negtive=True)
            # fake_output = netC(fake_feats, sent_emb)
        # # MA-GP
        ...
        # whole D loss
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            # errD_h = errD_real + (errD_fake + errD_mis)/2.0 + errD_MAGP
            errD_rf = cf_loss_fn(real_output, fake_output)
            errD_rmis = cf_loss_fn(real_output, real_mismatch_output)
            # errD = - errD_rf
            errD = - errD_rf - errD_rmis
            # errD = - errD_rf + errD_h
        # update D
        if args.mixed_precision:
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            ...
            fake_output = netC(fake_feats, sent_emb)
            # fake_emb_output = netC(fake_feats, fake_emb)
            # mis_sent_emb = torch.cat((sent_emb[1:], sent_emb[0:1]), dim=0).detach()
            # fake_mismatch_output = netC(fake_feats, mis_sent_emb)
            # output, fake_output = netC(fake_feats, sent_emb)
            # mis_sent_emb = torch.cat((sent_emb[1:], sent_emb[0:1]), dim=0).detach()
            # fake_mismatch_output = netC(fake_feats, mis_sent_emb)
            errG_rf = cf_loss_fn(real_output.detach(), fake_output)
            # errG_ff = cf_loss_fn(fake_output, fake_emb_output)
            # errG_fmis = cf_loss_fn(fake_output, fake_mismatch_output)
            t = torch.randn(512, sent_emb.shape[1], device='cuda') * (0.05/sent_emb.shape[1] ** 0.5)
            ty_fake = torch.matmul(fake_emb, t.t())
            ty_sent = torch.matmul(sent_emb, t.t())
            err_emb = cf_loss_fn(ty_fake, ty_sent)
            text_img_sim = torch.cosine_similarity(fake_emb, sent_emb).mean()
            errG = errG_rf - args.sim_w * text_img_sim
            # errG = errG_rf - args.sim_w * text_img_sim + err_emb * 0.1
            # errG = -output.mean() + errG_rf - args.sim_w*text_img_sim
        if args.mixed_precision:
        ...
        writer.add_scalar('errD', errD.item(), iteration)
        # writer.add_scalar('errD_h', errD_h.item(), iteration)
        # writer.add_scalar('errD_rf', errD_rf.item(), iteration)
        # writer.add_scalar('errD_rmis', errD_rmis.item(), iteration)
        writer.add_scalar('real_output', real_output.abs().mean(), iteration)
        # writer.add_scalar('real_mismatch_output', real_mismatch_output.abs().mean(), iteration)
        writer.add_scalar('errG', errG.item(), iteration)
        # writer.add_scalar('errG_rf', errG_rf.item(), iteration)
        # writer.add_scalar('errG_h', output.mean().item(), iteration)
        writer.add_scalar('fake_output', fake_output.abs().mean(), iteration)
        # writer.add_scalar('fake_mismatch_output', real_mismatch_output.abs().mean(), iteration)
        writer.add_scalar('real_feats', real_feats.abs().mean(), iteration)
        writer.add_scalar('fake_feats', fake_feats.abs().mean(), iteration)
        iteration += 1
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
...
def sample_one_batch(noise, sent, netG, multi_gpus, epoch, img_save_dir, writer):
    if (multi_gpus==True) and (get_rank() != 0):
        ...
        image_grid = make_grid(fixed_results.data.cpu(), nrow=8, range=(-1, 1), normalize=True)
        writer.add_image('generated images', image_grid, epoch)
...
def predict_loss(predictor, img_feature, text_feature, negtive):
    output,output_t = predictor(img_feature, text_feature)
    err = hinge_loss(output, negtive)
    return output,err,output_t
