...
def get_fix_data(train_dl, test_dl, text_encoder, device, batch_size=None):
    fixed_image_train, _, _, fixed_sent_train, fixed_word_train, fixed_key_train = get_one_batch_data(train_dl, text_encoder, device, int(batch_size // 2))\
                                                                                                                                if batch_size is not None \
                                                                                                                                else get_one_batch_data(train_dl, text_encoder, device)
    fixed_image_test, _, _, fixed_sent_test, fixed_word_test, fixed_key_test= get_one_batch_data(test_dl, text_encoder, device, int(batch_size // 2)) \
                                                                                                                            if batch_size is not None \
                                                                                                                            else get_one_batch_data(test_dl, text_encoder, device)
    ...
    return fixed_image, fixed_sent, fixed_word


def get_one_batch_data(dataloader, text_encoder, device, batch_size=None):
    data = next(iter(dataloader[0])) if isinstance(dataloader, list) and len(dataloader) > 0 else next(iter(dataloader))
    data = tuple(d[:batch_size] for d in data) if batch_size is not None else data
...
def prepare_data(data, text_encoder, device, batch_size=None):
    data = tuple(d[:batch_size] for d in data) if batch_size is not None else data
