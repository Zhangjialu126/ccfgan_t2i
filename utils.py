...
def prepare_parser():
    ...
    parser.add_argument(
        ...
        help='Number of forward passes to use in accumulating standing stats? '
             '(default: %(default)s)')
    parser.add_argument('--t_sigma_num', type=int, default=64,
                        help='Number of sigma to use in t-net? Set to 0 to use fixed Gaussian'
                             '(default: %(default)s)')
    parser.add_argument("--unconditional", action="store_true", default=False)
    parser.add_argument('--t_dim', type=int, default=256, help='Dimension of output tx (default: %(default)s)')
    parser.add_argument("--require_classifier", action="store_true", default=False)
    parser.add_argument('--c_mode', type=str, default='adc',
                        help='Classifier mode for CCF-GAN in {ac | tac | adc}. (default: %(default)s)')
    parser.add_argument('--ce_mode', type=str, default='CE',
                        help='Cross entropy loss mode for CCF-GAN in {CE | D2DCE}. (default: %(default)s)')
    parser.add_argument('--CD_lambda', type=float, default=1.0,
                        help='Classification task lambda coefficient for D (default: %(default)s)')
    parser.add_argument('--CG_lambda', type=float, default=1.0,
                        help='Classification task lambda coefficient for G (default: %(default)s)')
    parser.add_argument('--npz_path', type=str, default='../data/birds/npz/bird_val256_FIDK0.npz',
...
def add_sample_parser(parser):
    ...
    parser.add_argument(
        '--recons', action='store_true', default=False,
        help='Test reconstructions? (default: %(default)s)')
    return parser
...
nclass_dict = {'I32': 1000, 'I32_hdf5': 1000,
               ...
               'C10': 10, 'C100': 100,
               'L64': 1, 'L128': 1, 'A64': 1,
               'A128': 1, 'I_tiny': 200, 'VGGFace2': 8631,
               'V200': 200, 'V500': 500, 'V1000': 1000,
               'CUB200': 200, 'SVHN': 10,
               'CUB128': 200, 't2i_birds': 200}
...
classes_per_sheet_dict = {'I32': 50, 'I32_hdf5': 50,
                          ...
                          'C10': 10, 'C100': 100,
                          'L64': 1, 'L128': 1, 'A64': 1,
                          'A128': 1, 'I_tiny': 50, 'VGGFace2': 20,
                          'V200': 20, 'V500': 20, 'V1000': 20,
                          'CUB200': 20, 'SVHN': 10,
                          'CUB128': 20, 't2i_birds': 20}
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64, test_batch_size=64,
    ...
    dataset_kwargs = {'index_filename': '%s_imgs.npz' % dataset, 'load_in_mem': load_in_mem}
    ...
    if 'hdf5' in dataset:
        ...
        if augment:
            ...
            if dataset in ['C10', 'C100']:
                train_transform = []
            elif dataset in ['L64', 'L128', 'A64', 'A128']:
                train_transform = [transforms.Resize(image_size), transforms.CenterCrop(image_size)]
                if dataset in ['L64', 'L128']:
    ...
    if dataset in ['SVHN']:
        train_set = which_dataset(root=data_root, transform=train_transform, download=True)
    elif dataset in ['t2i_birds']:
        ...
        train_set = which_dataset(split='train', transform=train_transform, args=t2i_kwargs)
        val_set = which_dataset(split='test', transform=train_transform, args=t2i_kwargs)
    else:
        train_set = which_dataset(root=data_root, transform=train_transform, **dataset_kwargs)
    ...
    #             indices.append(i)
    #     idx = indices
    #     train_set = Subset(train_set, [i for i in idx])
    #     print(len(train_set))
    #########################################################

    # if dataset in ['VGGFace2']:
    #     list_n = random.sample(range(8631), 200)
    #     indices = []
    #     print('list_n', list_n)
    #     print('n classes', len(train_set.imgs))
    #     for i in range(len(train_set.imgs)): # targets
    #         if int(train_set.imgs[i][1]) in list_n:
    #             indices.append(i)
    #     idx = indices
    #     train_set = Subset(train_set, [i for i in idx])
    #     print('chosen classes', len(indices))
    # indices = []
    # print(len(train_set.imgs))
    # for i in range(len(train_set.imgs)): # targets
    #     if int(train_set.imgs[i][1]) < 500:
    #         indices.append(i)
    # idx = indices
    # train_set = Subset(train_set, [i for i in idx])
    # print(len(indices))
    ...
    if use_multiepoch_sampler:
        ...
        test_loader = DataLoader(val_set, batch_size=test_batch_size,
                                  shuffle=shuffle, **loader_kwargs)
...
def name_from_config(config):
    name = '_'.join([
        item for item in [
            ...
            'GHier' if config['G_shared'] and config['hier'] else None,
            'ema' if config['ema'] else None,
            'Uncond' if config['unconditional'] else None,
...
def interpolated_imgs(G, D, x, num_of_groups, config, samples_root, experiment_name, folder_number, device='cuda'):
    with torch.no_grad():
        # this function is used for interpolation between images
        if config['parallel']:
            D_real = nn.parallel.data_parallel(D, x[0:128, :, :, :])
        else:
            D_real = D(x)

        # interpolation
        interp_imgs = torch.zeros([num_of_groups * 10, x.shape[1], x.shape[2], x.shape[3]], device=device)
        interp_feature = torch.zeros([num_of_groups * 8, D_real.shape[1]], device=device)
        for i in range(num_of_groups):
            for j in range(8):
                alpha = j / 7
                interp_feature[(i - 1) * 8 + j, :] = alpha * D_real[(i - 1) * 2 + 1, :] \
                                                     + (1 - alpha) * D_real[(i - 1) * 2, :]

        # interp_feature = nn.functional.normalize(interp_feature, p=2, dim=1)
        y = torch.zeros(interp_feature.shape[0], dtype=torch.long, device=device)
        if config['parallel']:
            x_fake = nn.parallel.data_parallel(G, (interp_feature, G.shared(y)))
        else:
            x_fake = G(interp_feature, G.shared(y))

        for i in range(num_of_groups):
            interp_imgs[(i - 1) * 10, :, :, :] = x[(i - 1) * 2, :, :, :]
            interp_imgs[(i - 1) * 10 + 9, :, :, :] = x[(i - 1) * 2 + 1, :, :, :]
            for j in range(8):
                interp_imgs[(i - 1) * 10 + j + 1, :, :, :] = x_fake[(i - 1) * 8 + j, :, :, :]
        # plot result
        image_filename = '%s/%s/%d/interp_imgs.jpg' % (samples_root, experiment_name,
                                                       folder_number)
        ...
        # y = torch.zeros(interp_feature.shape[0], dtype=torch.long, device=device)
        # if config['parallel']:
        #     x_fake = nn.parallel.data_parallel(G, (D_real, G.shared(y)))
        # else:
        #     x_fake = G(D_real, G.shared(y))
        #
        # # plot result
        # image_filename = '%s/%s/%d/rec_imgs.jpg' % (samples_root, experiment_name,
        #                                             folder_number)
        # torchvision.utils.save_image(x_fake.cpu(), image_filename, nrow=8, normalize=True)
        # image_filename = '%s/%s/%d/ori_imgs.jpg' % (samples_root, experiment_name,
        #                                             folder_number)
        # torchvision.utils.save_image(x[0:128, :, :, :].cpu(), image_filename, nrow=8, normalize=True)


def sample_rec(G, D, y_, loader, device, config):
    with torch.no_grad():
        x = next(iter(loader))[0].to(device)
        y_.sample_()
        if config['parallel']:
            D_real = nn.parallel.data_parallel(D, x).data
        else:
            D_real = D(x)

        if config['parallel']:
            G_z = nn.parallel.data_parallel(G, (D_real, G.shared(y_))).data
        else:
            G_z = G(D_real, G.shared(y_))

        return G_z, y_

def D_outch(ch=64):
    arch = {}
    arch[256] = {'out_channels': 16 * ch}
    arch[128] = {'out_channels': 16 * ch}
    arch[64] = {'out_channels': 16 * ch}
    arch[32] = {'out_channels': 4 * ch}
    return arch