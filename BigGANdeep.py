    ...
    def __init__(self, G_ch=64, G_depth=2, dim_z=128, bottom_width=4, resolution=128,
        ...
        for index in range(len(self.arch['out_channels'])):
            self.blocks += [[GBlock(in_channels=self.arch['in_channels'][index],
                                    out_channels=self.arch['out_channels'][index] if g_index == 0 else
...
class Discriminator(nn.Module):
    ...
    def __init__(self, D_ch=64, D_wide=True, D_depth=2, resolution=128,
        ...
        if not self.unconditional:
            self.y_embed = self.which_embedding(self.n_classes, self.arch['out_channels'][-1])
            self.linear_hy = self.which_linear(self.arch['out_channels'][-1], self.n_classes)
            self.embed = self.which_embedding(self.n_classes, self.n_classes)
            self.linear_proj = self.which_linear(self.arch['out_channels'][-1], self.n_classes)
...
class G_D(nn.Module):
    def __init__(self, G, D, t_dim=128, **kwargs):
        super(G_D, self).__init__()
        self.G = G
        self.D = D
        self.output_dim = t_dim
        self.unconditional = kwargs["unconditional"]
    ...
    def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False,
        ...
        if split_D:
            if self.unconditional:
                ...
                D_fake, h_x = self.D(G_z, gy)
            if x is not None:
                if self.unconditional:
                    D_real = self.D(x, dy)
                else:
                    D_real, h_target = self.D(x, dy)
                if self.unconditional:
                    return D_fake, D_real
                else:
                    return D_fake, D_real, h_x, h_target
            else:
                if return_G_z:
                    return D_fake, G_z
                else:
        ...
        # along the batch dimension for improved efficiency.
            ...
            if self.unconditional:
                ...
                h_x, h_target = torch.split(h_out, [G_z.shape[0], x.shape[0]])
            if x is not None:
                if self.unconditional:
                    return torch.split(D_out, [G_z.shape[0], x.shape[0]])  # D_fake, D_real
                else:
                    return D_fake, D_real, h_x, h_target  # D_fake, D_real
            else:
                if return_G_z:
                    return D_out, G_z
                else:
